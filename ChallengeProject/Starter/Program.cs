...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "4":
            ...
            // Looks at all the pets one by one
            for (int i = 0; i < maxPets; i++)
            {
                // Makes sure that the current animal index is not empty (default values).
                if (ourAnimals[i, 0] != "ID #: ")
                {
                    ...
                    if (ourAnimals[i, 3].Trim() == "Nickname:")
                    {
                        ...
                        do
                        {
                            ...
                            if ((readResult != null) && (readResult != "")) validEntry = true;
                            Console.WriteLine();

                        } while (validEntry != true);

                        // Saves the new nickname
                        ourAnimals[i, 3] = "Nickname: " + readResult;
                    }

                    // Checks if the personality is valid
